...
from app.dto.dto import AppUserReadSerializer
...
def register_user(request, group_name: str, permissions: list[str]):
    ...
    with transaction.atomic():
        ...
        return Response({"details": "User successfully created", "user": AppUserReadSerializer(user).data}, status.HTTP_200_OK)
...
class RegisterTrainingUserView(APIView):
    ...
    @staticmethod
    def post(request):
        return register_user(request, group_name='app.app_user',
                             permissions=['app.user_order_view'])
